									...
									<table
										...
										<thead>
											<tr>
												...
												<th>Ciclos promedio (Hr/vtas)</th>
												<th>Produccion media x hora (Ton/hrs)</th>
												<th>Tiempo ciclos (Hr/vtas)</th>
												<th>Velocidad promedio (Km/hr)</th>
												<th>Distancia PromedioXcamión</th>
												<th>Vueltas x camion</th>
												<th>RalentiXcamion (Hrs/camion)</th>
										...
										<tbody>
											...
											<tr class="fila">
												...
												<td><%=df.format(c.getTransp_kmxh()) %></td>
												...
												<c:if test="${cform.origen == 0}"><input type="hidden" class="id_geo" value="<%=c.getId_geo()%>"></input></c:if>
											</tr>
					...
					<table class='table table-bordered table-hover dataTables-example2 ' id="example" data-filter=#filter>
						<thead>
							<tr>
								...
								<th>Vueltas totales</th>
								<th>Produccion media x hora  (Ton/hrs)</th>
								<th>Tiempo de ciclo promedio (Hr/vtas)</th>
								<th>Velocidad promedio  (Km/hr)</th>
								<th>Distancia Promedio x camión</th>
							</tr>
						</thead>
						<tbody>
						</tbody>
...
<script type="text/javascript">
	...
	function ajaxlista3(idDestino,vturno,vorigen) {
		
		var vfecha = document.getElementById("fechaHidden").value;
		console.log("turno : " +vturno + ",origen : " + vorigen + ", fecha: "+vfecha );
		var titulo = document.getElementById("destino"+idDestino).value;
	...
	$(document).ready(function() {
		...
		$('#tabla tbody').on('click', 'tr.fila', function() {
			...
			if (row.child.isShown()) {
				...
				if(vorigen == 0){
					vorigen = $(this).find("input.id_geo").val(); 				
				}
