...
<body class="top-navigation">
            ...
            <div class="row"> <!-- Bloque Formulario de busqueda --> 
                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                      ...
                      <form:form action="reportetiempoconduccion.html" commandName="reportePatenteFechaForm"  id="data_1" autocomplete="off">                  
													...
													<c:if test="${grupo.gruId != null }">
                                                     ...
                                                     <div class="col-lg-2  ">   
                                                      <div class="form-group">                                      
                                                              ...
                                                              <div>
                                                               <select  name="conductor" id="conductor"  style="width:100%;" tabindex="4"  data-placeholder="Seleccione un Conductor" class="form-control text-uppercase">
																					
															<option value=""></option>	
														
																																								
														<c:forEach items="${listconductores}" var="c" varStatus="count" >	  
														
														   <c:if test="${not  empty c }">	
														      
															 <option value="${c.rut}" <c:if test="${c.rut == rform.conductor}"> selected</c:if>>${c.nombre} ${c.apellido}</option>																	
								...
								<div class="table-responsive"> <!-- class="table table-striped table-bordered table-hover dataTables-example"> -->
									<table
										...
										<thead>
                                            ...
                                            <th>Duracion</th>
                                            <th>TPO Detenido</th>                                            
                                            <th>TPO en Movimiento</th>
                                            <th>Distancia (KM)</th>
                                            <th>Velocidad Promedio</th>
                                            <th>Ubicacion</th>                                        
                                            <th>Mapa</th>   
                                             <th>Duracion Seg</th>
                                            <th>TPO Detenido Seg</th>                                            
                                            <th>TPO en Movimiento Seg</th>                                                                                                                                        
											</tr>
										</thead>
									 <tbody>
				    ...
				    <c:if test="${not  empty rlist }">	    
						...
						<c:forEach items="${rlist}" var="r" varStatus="count" >	  
		                                	...
		                                	<tr>
		                                        ...
		                                        <td>${r.conductor}</td>														                                    
		                                        <td><fmt:formatDate value="${r.inicio}" pattern="dd/MM/yyyy HH:mm:ss" /> </td>		                                       
		                                        <td><fmt:formatDate value="${r.fin}" pattern="dd/MM/yyyy HH:mm:ss" /> </td> 		                                                         
		                                        <td>${r.tiempoconduccionS}</td>	
		                                        <td>${r.timpoignicionS}</td>	
		                                        <td>${r.tiempomovimientoS}</td>	               		                                    							                                      				                                     		                                        
		                                        <td><fmt:formatNumber type = "number"   maxFractionDigits = "2" value = "${r.distancia}" /></td>
		                                        <td><fmt:formatNumber type = "number"   maxFractionDigits = "2" value = "${r.velpro}" /></td>
		                                        <td>${r.ubicacionfinal}</td>
		                                          <td title="Ver en Mapa"><font size="+1.5"><a style="color:#337ab7" class="link_mapa" link="maparuta.html?patente=${r.patente}&fechain=<fmt:formatDate value="${r.inicio}" pattern="dd/MM/yyyy HH:mm:ss" /> &fechafin=<fmt:formatDate value="${r.fin}" pattern="dd/MM/yyyy HH:mm:ss" /> &lat=${r.lat}&lon=${r.lon}"><i class="fa fa-external-link"></i></a></font></td>
		                                         <td><fmt:formatNumber type = "number"   maxFractionDigits = "2" value = "${r.tiempoconduccion}" /></td>
		                                         <td><fmt:formatNumber type = "number"   maxFractionDigits = "2" value = "${r.timpoignicion}" /></td>
		                                         <td><fmt:formatNumber type = "number"   maxFractionDigits = "2" value = "${r.tiempomovimiento}" /></td>	
  ...
  //requiere validarFechaHoraJs y sweetalert
    ...
    $( "#conductor" ).select2( {
	 ...
	 $(document).off("change","#patente").on("change","#patente", function()
            {                   
                ...
                $('#conductor').prop('selectedIndex', 0);  $('#conductor').select2();
            });
    ...
    $(document).off("change","#conductor").on("change","#conductor", function()
            { 
    	  $('#faena').prop('selectedIndex', 0);  $('#faena').select2();     
    	  $('#patente').prop('selectedIndex', 0); $('#patente').select2();
            });
    ...
    $(document).on("click", ".btn_submit", function(e){ 
 		if($("#faena").val()=="" && $("#patente").val()=="" && $("#conductor").val()==""){            	    
 	    	swal("", "Debe seleccionar una Faena o una Patente o un Conductor ", "error"); $("#faena").focus();
 	    	return  false;
 	    }
 	    else{ 	    	
        ...
        $('.dataTables-example').DataTable({
            ...
            buttons: [
                ...
                {extend: 'pdf', 
                	title: 'ReporteTiempoConduccion',                    	
               	 exportOptions: {
                        columns: [ 0, 1, 2, 3, 4 ,5,5,6,7,8,9,10,11  ]
               	 }
...
<%-- 
 ...
 <c:if test="${r.enconduccion ne 'SI' }">
		                                        <td><fmt:formatDate value="${r.fin}" pattern="dd/MM/yyyy HH:mm:ss" /> </td> 
		                                        </c:if>
		                                        <c:if test="${r.enconduccion eq 'SI' }">
		                                        <td><spam style='color: red'> En conducción</spam> </td> 
		                                        </c:if>    
