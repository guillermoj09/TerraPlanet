...
@Controller
public class UltimaPosicionAjaxDController {
	
	
	final static Logger logger = Logger.getLogger(UltimaPosicionAjaxDController.class);
							  
	@RequestMapping(value = "/ultimaposicionajaxd" , method = RequestMethod.POST) // produces = "application/json"
	 //List<StringResponse> en caso que sea una lista como respuesta
	public @ResponseBody List<MapaUltimoGPSJson> ultimaposicionajaxd( HttpServletRequest request, HttpServletResponse response) {
		...
		if(tipo.equals("gps")){
			...
			mulist = ReporteService.findVehiculoUltimaPosicionAjaxD(rutcliente, rut);
				
		}else if (tipo.equals("db")){
			
			mulist = ReporteService.findVehiculoUltimaPosicionAjaxD2(rutcliente, rut);
