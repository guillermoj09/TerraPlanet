...
@Controller
public class GeocercaUltimaGPSListadoAjaxController {
	
	
	//final static Logger logger = Logger.getLogger(GeocercaUltimaGPSListadoController.class);
							  
	@RequestMapping(value = "/geocercaultimagpslistadoajax" , method = RequestMethod.POST) // produces = "application/json"
	...
	public @ResponseBody List<MapaUltimoGPSJson> ultimaposicionajax( HttpServletRequest request, HttpServletResponse response) {
		...
		String nombregeo =  request.getParameter("nombregeo");

				
		List<MapaUltimoGPSJson>  glist = new ArrayList<MapaUltimoGPSJson>();
				
		glist = ReporteService.findUltimaPosicionGeoAjax(rutcliente, nombregeo);
