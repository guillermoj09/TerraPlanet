                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        <div class="ibox-title">
                            <h5>Menus x Modulo (${modulo.modNombre})</h5>
                        	...
                        	<div class="col-12 " style="padding-bottom:20px;padding-top:10px">                                                                                                              
                                       <a class="btn btn-primary" href="crearmenu?id=${modulo.modId}">Nuevo <i class="fa fa-plus"></i></a>
			                     </div> 
				    ...
				    <c:if test="${not  empty menus }">	    
						...
						<c:forEach items="${menus}" var="m" varStatus="count" >	  
		                                     ...
		                                     <c:if test="${m.menIdMen == 0 }">
		                                      <tr>
		                                      
		                                      <td onClick="location.href='editarmenu?id=${m.menId}'" title="Editar Menu">${m.menNombre}</td>
		                                        <td onClick="location.href='editarmenu?id=${m.menId}'" title="Editar Menu">${m.menLink}</td>
		                                             <td  onClick="location.href='editarmenu?id=${m.menId}'" title="Editar Menu">N/A</td>
		                                	...
		                                	<tr>
		                                        ...
		                                        <td onClick="location.href='editarmenu?id=${m.menId}'" title="Editar Menu Padre">${m.menNombre}</td>
		                                        <td onClick="location.href='editarmenu?id=${m.menId}'" title="Editar Menu Padre">${m.menLink}</td>
		                                          
		                                        <td onClick="location.href='consultarmenuxpadre?id=${m.menId}'" title="Agregar menus hijos">SI -
		                                           <c:if test="${not  empty m.submenus }">	  
		                                        	<c:forEach items="${m.submenus}" var="s" varStatus="count" >		                                        
		                                        		<b>${s.menNombre}</b> -	                                        
		                                        	</c:forEach>		                  
		                                            </c:if>                
		                                        </td>	
